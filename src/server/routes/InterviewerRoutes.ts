...
interviewerRouter.get('/sequences/:sequenceId', async (ctx)=>{
...
interviewerRouter.get('/items/:itemId', async(ctx)=>{
	ctx.body = await itemsService.getItem(ctx.params.itemId);
});

interviewerRouter.get('/assets', async(ctx)=>{
	ctx.body = await assetsService.assetsList();
});
interviewerRouter.get('/assets/:assetId', async(ctx)=>{
...
interviewerRouter.post('/items', koaBody(), async (ctx)=>{
...
interviewerRouter.post('/assets', koaBody(), async(ctx)=>{
